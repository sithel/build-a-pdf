function buildPdfEntries(entries, value, el1) {
  entries.forEach( d => {
    ...
    let otherTag = (d[3].size == 0) ? 'tag-all show-tag-all' : ''
...
+ tags.reduce( (acc, t) => { return acc+`
  ...
  `}, "") + `
    ...
    row.setAttribute("class", ["list-group-item",otherTag,"category-"+value, ifTag(d[3], 'a', 'tag-'),ifTag(d[3], 'b', 'tag-'),ifTag(d[3], 'c', 'tag-'),ifTag(d[3], 'd', 'tag-'),ifTag(d[3], 'e', 'tag-'),ifTag(d[3], 'a', 'show-tag-'),ifTag(d[3], 'b', 'show-tag-'),ifTag(d[3], 'c', 'show-tag-'),ifTag(d[3], 'd', 'show-tag-'),ifTag(d[3], 'e', 'show-tag-')].join(" "),)
